...
/**
 * Интерфейс для проверяльщиков прав доступа пользователя к системе.
 * Объявляет методы класса PermissionCheckerBean, выполняющие проверку
 * разрешенности элементарных операций для конечного пользователя, по
...
public interface PermissionCheckerRemote {
	...
	/**
	 * Проверяет право модификации привилегий (администратор, создатель соревнований)
	 ...
	 * @return true если пользователь может использовать эту задачу в своеи контесте, иначе
	 * false.
	 */
