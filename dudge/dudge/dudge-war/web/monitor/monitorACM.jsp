<script type="text/javascript">
    ...
    Ext.onReady(function(){
        ...
        ds = new Ext.data.Store({
            ...
            proxy: new Ext.data.HttpProxy({
                url: 'monitor.do?reqCode=getAcmMonitorData'
            }),
    ...
    <c:forEach items="${contestLocal.contests}" var="contest">
        <c:if test="${contest.contestId == contestId}">
            ...
            <c:forEach items="${contestProblems}" var="contestProblem">
                                'problem' + '${contestProblem.problemMark}',
            </c:forEach>
        </c:if>  
    </c:forEach>
                    'time'
    ...
    <c:forEach items="${contestLocal.contests}" var="contest">
        <c:if test="${contest.contestId == contestId}">
            ...
            <c:forEach items="${contestProblems}" var="contestProblem">
                            {
                                header: '${contestProblem.problemMark}',
                                dataIndex: 'problem' + '${contestProblem.problemMark}',
